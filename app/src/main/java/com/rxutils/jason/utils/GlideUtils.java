...
public class GlideUtils {
    ...
    public static void loadImageView(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).diskCacheStrategy(DiskCacheStrategy.RESULT).error(R.mipmap.ic_default1).into(mImageView);
    }
    ...
    public static void loadImageViewLoding(Context mContext, String path, ImageView mImageView, int lodingImage, int errorImageView) {
        Glide.with(mContext).load(path).diskCacheStrategy(DiskCacheStrategy.RESULT).placeholder(lodingImage)
                .error(errorImageView).crossFade().into(mImageView);

    }

    public static void loadImageDefault(Context context, String path, ImageView imageView) {
        Glide.with(context).load(path).diskCacheStrategy(DiskCacheStrategy.RESULT).placeholder(R.mipmap.ic_default1)
                .error(R.mipmap.ic_default1).crossFade().into(imageView);
    }
